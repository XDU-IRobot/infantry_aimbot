...
static const std::vector<cv::Point3f> kBigArmorPoints{
    cv::Point3f{-ArmorSolver::kBigArmorWidth / 2,
                -ArmorSolver::kBigArmorHeight / 2, 0.f},
    cv::Point3f{ArmorSolver::kBigArmorWidth / 2,
                -ArmorSolver::kBigArmorHeight / 2, 0.f},
    cv::Point3f{ArmorSolver::kBigArmorWidth / 2,
                ArmorSolver::kBigArmorHeight / 2, 0.f},
...
static const std::vector<cv::Point3f> kSmallArmorPoints{
    cv::Point3f{-ArmorSolver::kSmallArmorWidth / 2,
                -ArmorSolver::kSmallArmorHeight / 2, 0.f},
    cv::Point3f{ArmorSolver::kSmallArmorWidth / 2,
                -ArmorSolver::kSmallArmorHeight / 2, 0.f},
    cv::Point3f{ArmorSolver::kSmallArmorWidth / 2,
                ArmorSolver::kSmallArmorHeight / 2, 0.f},
...
ArmorSolver::ArmorSolver(const RosParams& config)
    : R_gimbal2world_{Eigen::Matrix3d::Identity()} {
  // 相机内参
  Eigen::Matrix<double, 3, 3, Eigen::RowMajor> cam_mat(
      config.camera_info.camera_matrix.data());
  cv::eigen2cv(cam_mat, camera_matrix_);

  Eigen::Matrix<double, 1, 5> dist_vec(
      config.camera_info.distortion_coefficients.data());
  cv::eigen2cv(dist_vec, distort_coeffs_);

  // 外参
  R_gimbal2imubody_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
      config.solver.R_gimbal2imubody.data());
  R_camera2gimbal_ = Eigen::Matrix<double, 3, 3, Eigen::RowMajor>(
      config.solver.R_camera2gimbal.data());
...
void ArmorSolver::Solve(Armor& armor) const {
  ...
  // 图像点：{左上, 右上, 右下, 左下}
  std::vector<cv::Point2f> image_points{armor.left_light.up,
                                         armor.right_light.up,
                                         armor.right_light.down,
                                         armor.left_light.down};

  cv::Vec3d rvec, tvec;
  cv::solvePnP(object_points, image_points, camera_matrix_, distort_coeffs_,
  ...
  bool is_balance = (armor.kind == ArmorKind::kBig) &&
                    (armor.name == ArmorName::kThree ||
                     armor.name == ArmorName::kFour ||
                     armor.name == ArmorName::kFive);
  if (!is_balance) {
    OptimizeYaw(armor);
  }
}
...
void ArmorSolver::OptimizeYaw(Armor& armor) const {
  ...
  double yaw0 =
      tools::LimitRad(gimbal_ypr[0] - kSearchRange / 2 * CV_PI / 180.0);
...
double ArmorSolver::ArmorReprojectionError(const Armor& armor,
                                            double yaw) const {
  auto image_points =
      ReprojectArmor(armor.xyz_in_world, yaw, armor.kind, armor.name);
  double error = 0.0;
  std::vector<cv::Point2f> actual_points{armor.left_light.up,
                                          armor.right_light.up,
                                          armor.right_light.down,
                                          armor.left_light.down};
  for (int i = 0; i < 4; i++) {
...
std::vector<cv::Point2f> ArmorSolver::ReprojectArmor(
    const Eigen::Vector3d& xyz_in_world, double yaw, ArmorKind kind,
  ...
  // 前哨站pitch朝下，其余朝上
  double pitch =
      (name == ArmorName::kOutpost) ? -15.0 * CV_PI / 180.0 : 15.0 * CV_PI / 180.0;
  ...
  Eigen::Matrix3d R_armor2camera = R_camera2gimbal_.transpose() *
                                    R_gimbal2world_.transpose() *
                                    R_armor2world;
