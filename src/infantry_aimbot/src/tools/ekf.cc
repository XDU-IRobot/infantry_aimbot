...
Eigen::VectorXd ExtendedKalmanFilter::Predict(const Eigen::MatrixXd& F,
  ...
  return Predict(F, Q,
                 [&](const Eigen::VectorXd& x) { return F * x; });
}
...
Eigen::VectorXd ExtendedKalmanFilter::Update(
    ...
    std::function<Eigen::VectorXd(const Eigen::VectorXd&,
        ...
        z_subtract) {
  return Update(z, H, R,
                [&](const Eigen::VectorXd& x) { return H * x; }, z_subtract);
}
...
Eigen::VectorXd ExtendedKalmanFilter::Update(
  ...
  if (nis > kNisThreshold) nis_count_++, data["nis_fail"] = 1;
  if (nees > kNeesThreshold) nees_count_++, data["nees_fail"] = 1;
  total_count_++;
  last_nis = nis;
