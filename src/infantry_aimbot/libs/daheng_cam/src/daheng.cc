...
namespace camera {
DahengCam::DahengCam(const std::string camera_sn) : camera_sn_(camera_sn), device_(nullptr), pFrameBuffer_(nullptr) {
  // default param
...
bool DahengCam::is_open() { return is_open_; }
...
bool DahengCam::grab_image(cv::Mat &image) {
  ...
  VxInt32 DXstatus = DxRaw8toRGB24(pFrameBuffer_->pImgBuf, image.data, pFrameBuffer_->nWidth, pFrameBuffer_->nHeight,
                                   RAW2RGB_NEIGHBOUR,
...
bool DahengCam::ResetParameters(const std::map<CamParamType, int> &param_map) {
  this->close();
  for (auto it = param_map.begin(); it != param_map.end(); it++) this->set_parameter(it->first, it->second);
  return this->open();
}
