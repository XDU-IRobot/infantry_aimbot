...
inline std::vector<int> HungarianAssign(size_t rows, size_t cols, CostFunc cost, double INF = 1e18) {
  ...
  for (size_t i = 1; i <= m; ++i) {
    ...
    do {
      ...
      for (size_t j = 1; j <= n; ++j)
        if (!used[j]) {
          double cur =
              cost(static_cast<size_t>(i0 - 1), static_cast<size_t>(j - 1)) - u[static_cast<size_t>(i0)] - v[j];
          if (cur < minv[j]) {
            minv[j] = cur;
  ...
  for (size_t j = 1; j <= n; ++j)
    if (p[j] != 0) assign[static_cast<size_t>(p[j] - 1)] = static_cast<int>(j - 1);
  return assign;
}
...
inline std::vector<std::pair<size_t, size_t>> MinCostPairs(const LeftRange& left, const RightRange& right,
                                                           CostFn costFn, double INF = 1e18) {
